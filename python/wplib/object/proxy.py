...
- ProxyLink class handles any logic to modify reference to proxy
target between the base and result
also avoids polluting proxy namespace too much


probably the most complex thing I've ever done, but for what it does, it could be worse
...
class Proxy(#ABC,
	...
	_proxyAttrs = ("_proxyLink", "_proxyResult", "_proxyBase", "_proxyData", "_proxyStrongRef")
	#_proxyObjKey = "_proxyResult"


	def __init__(self, obj, proxyLink:ProxyLink, proxyData=None):
	...
	def _createClassProxy(cls, targetCls):
		...
		"""
		#print("new proxy class", cls, targetCls)
		...
		# right now every base method is fully wrapped as a proxy method,
		# can this be restricted to only explicitly defined/overridden methods?

		for methodName in toWrap:
			# do not override methods if they appear in class
			if hasattr(targetCls, methodName) \
					and not methodName in dir(cls):
	...
	@classmethod
	def getProxy(cls, targetObj, proxyLinkCls=None,
	             shared=True,
	             proxyData=None,
		...
		proxyObj = cls(targetObj, proxyLink=linkObj,
		               proxyData=proxyData
		               )
	...
	def __class__(self):
		...
		return self._proxyLink.baseClass
...
"""consider some way to convert an entire complex object into a hierarchy of proxies,
in-place"""
...
if __name__ == '__main__':
	...
	# directVal = 3
	# directProxy = DirectProxy(directVal)

	#print(directProxy, type(directProxy))
