...
// editing to add simple compatibility with eigen types
#include <Eigen/Dense>
...
namespace aabb
{
    ...
    AABB::AABB(const Eigen::Vector3f& lowerBound_, const Eigen::Vector3f& upperBound_)
    {
        AABB::AABB(
            std::vector<double>{ lowerBound_.x(), lowerBound_.y(), lowerBound_.z() },
            std::vector<double>{ upperBound_.x(), upperBound_.y(), upperBound_.z() }
        );
    }
