...
intersections should be indexed by anchor and UVN, at least for points -
a common intersectionPoint should hold uvns of all contributing anchors
*/
...
[ uvnA : anchor 1, uvnB : anchor 2, curveIntersection3 : neighbour 1 ]
...
from an element's DIRECT ANCHORS, we can get spatial data to query?
plus AABB checks for surfaces, curves + surfaces
*/

Status& _updatePointAnchorIntersections(
	Status& s,
	...
	*/
	/* only called when we know for sure point has a anchor */
	SPoint& pData = manifold.pDataMap[el->name];
	...
	//SPointAnchorData& anchorData = pData.anchorData; // only single anchor for points
	...
	SElement* anchorEl = manifold.getEl(el->anchors[0]);
	SPointAnchorData& anchorData = pData.anchorData; // only single anchor for points

	switch (anchorEl->elType) {
	case SElType::point: { // point-point intersection, just a single point

		ptr->elUVNMap[anchorEl->globalIndex].push_back(Vector3f(0, 0, 0));
		record.elUVNPointMap[anchorEl->globalIndex][Vector3i(0, 0, 0)] = ptr->index;

		record.elMap[el->globalIndex][anchorEl->globalIndex].push_back({ ptr->index, Intersection::POINT });
		record.elMap[anchorEl->globalIndex][el->globalIndex].push_back({ ptr->index, Intersection::POINT });
	...
	case SElType::edge: {
		/* edge driving point
		*/
		SEdge& dEData = manifold.eDataMap[anchorEl->name];

		auto found = ptr->elUVNMap.find(anchorEl->globalIndex);
		if (found == ptr->elUVNMap.end()) { /* edge not already included in point*/
			ptr->elUVNMap.insert({ anchorEl->globalIndex, {anchorData.uvn} });
		}
		else {
			...
			*/
			...
			auto newFound = std::find(found->second.begin(), found->second.end(), anchorData.uvn);
			if (newFound == found->second.end()) {
				found->second.push_back(anchorData.uvn);
			}
		}

		record.elUVNPointMap[anchorEl->globalIndex][toKey(anchorData.uvn)] = ptr->index;
...
Status& _updateEdgeVertices(
	...
	*/
	SEdge& eData = manifold.eDataMap[el->name];
	...
	for (auto& p : record.elMap[el->globalIndex]) {
		...
		*/
		...
		for (auto& otherP : p.second) {
			...
			*/
			SEdge& otherEData = manifold.eDataMap[otherEl->name];
...
Status& _updateEdgeAnchorIntersections(
	Status& s,
	StrataManifold& manifold,
	...
	for (int i = 0; i < el->anchors.size(); i++) { // 
		int driveIdx = el->anchors[i];
		SElement* anchorEl = manifold.getEl(driveIdx);
		SEdgeAnchorData& anchorData = eData.anchorDatas[i];
		switch (anchorEl->elType) { 
		case SElType::point: {// point anchor of curve - intersection is point
			/* I think this should be guaranteed to be found already 
			as an IntersectionPoint, no?
			*/
			IntersectionPoint* ptr = record.getPointByVectorPosition(anchorData.pos());
			if(ptr == nullptr){
				ptr = record.newPoint(anchorData.pos());
			}

			// add anchor
			ptr->elUVNMap[anchorEl->globalIndex].push_back(anchorData.uvn);
			ptr->elUVNMap[el->globalIndex].push_back(Vector3f(anchorData.uOnEdge, 0, 0));
		...
		case SElType::edge: {
			...
			// add anchor
			ptr->elUVNMap[anchorEl->globalIndex].push_back(anchorData.uvn);
			ptr->elUVNMap[el->globalIndex].push_back({ anchorData.uOnEdge, 0, 0 });
	...
	Status& strata::updateElementIntersections(
		...
		*/
		switch (el->elType) {
			case SElType::point: {
				_updatePointAnchorIntersections(
					s,
					manifold,
					el,
					record
		...
		case SElType::edge: {
			_updateEdgeAnchorIntersections(
				s,
				manifold,
				el,
				record
...
Status& strata::elementGreaterThan(
	...
	*/
	...
	case SElType::edge: {
		...
		SEdge& baseData = manifold.eDataMap[elA->name];
		...
		*/
		...
		for (auto idB : idsB) {
			...
			SElement* anchorEl = manifold.getEl(idB);
			if (anchorEl == nullptr) {
				continue;
			}
			newName += anchorEl->name + "_";
		}
		...
		SEdge& newEdgeData = manifold.eDataMap[newName];
